...
//   const [accountType, setAccountType] = useState(ACCOUNT_TYPE.INSTRUCTOR)
  ...
  const handleBulkSignup = (instructors) => {
  ...
  const { firstName, lastName, email, password, confirmPassword,instructorId,department,accountType} = formData
  ...
  const handleOnSubmit = (e) => {
    ...
    dispatch(
      signUp(formData.firstName,formData.lastName,formData.email,formData.password,formData.confirmPassword, // contactNumber, assuming null if not used
        formData.department,formData.instructorId,formData.accountType)
    )
    ...
    <div>
      ...
      <form onSubmit={handleOnSubmit} className="flex w-full flex-col gap-y-4">
        ...
        <div className="flex gap-x-4">
          <label className="relative">
            <p className="mb-1 text-[0.875rem] leading-[1.375rem] text-richblack-5">
              Instructor ID<sup className="text-pink-200">*</sup>
            </p>
            <input
              required
              type="number"
